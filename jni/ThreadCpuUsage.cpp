...
bool ThreadCpuUsage::sampleAndEnable(double& ns)
{
    ...
    if (enable()) {
        ...
        mAccumulator = 0;
        LOGV("sampleAndEnable %.0f", ns);
        return true;
    } else {
        // first time called
        ns = 0.0;
        LOGV("sampleAndEnable false");
        return false;
    }
}
...
#define KERNEL_MAX_SIZE 12
        ...
        if (actual >= 2 && kernelMax[actual-1] == '\n') {
            ...
            if (sKernelMax >= MAX_CPU - 1) {
                LOGW("kernel_max %d but MAX_CPU %d", sKernelMax, MAX_CPU);
                sKernelMax = MAX_CPU;
            } else if (sKernelMax < 0) {
                LOGW("kernel_max invalid %d", sKernelMax);
                sKernelMax = 1;
            } else {
                ++sKernelMax;
                LOGV("number of CPUs %d", sKernelMax);
            }
        } else {
            LOGW("Can't read number of CPUs");
        }
        (void) close(fd);
    } else {
        LOGW("Can't open number of CPUs");
    }
    int i;
    for (i = 0; i < MAX_CPU; ++i) {
        sScalingFds[i] = -1;
...
uint32_t ThreadCpuUsage::getCpukHz(int cpuNum)
{
    if (cpuNum < 0 || cpuNum >= MAX_CPU) {
        LOGW("getCpukHz called with invalid CPU %d", cpuNum);
        return 0;
    }
...
#define FREQ_PATH "/sys/devices/system/cpu/cpu?/cpufreq/scaling_cur_freq"
        ...
        if (fd < 0) {
            LOGW("getCpukHz can't open CPU %d", cpuNum);
            return 0;
        }
    }
#define KHZ_SIZE 12
    ...
    if (ret != mCurrentkHz[cpuNum]) {
        if (ret > 0) {
            LOGV("CPU %d frequency %u kHz", cpuNum, ret);
        } else {
            LOGW("Can't read CPU %d frequency", cpuNum);
        }
